...
const MODULE_TEXT = { BA1: 'text-blue-400', BA2: 'text-emerald-400', BA3: 'text-violet-400', BA4: 'text-amber-400' }
    ...
    <Card icon={GraduationCap} title="CIMA Study Tracker">
      ...
      <div className="flex gap-1" role="tablist" aria-label="CIMA modules">
        {MODULES.map((m) => (
          <button key={m} role="tab" aria-selected={module_ === m} onClick={() => switchModule(m)}
            className={`num flex-1 rounded-lg py-2 min-h-[44px] text-sm font-bold ${press} ${focusRing} ${
              module_ === m ? `${MODULE_COLOR[m]} text-white` : 'bg-card2 text-mut hover:bg-veil'
            }`}>
            {m}
          </button>
        ))}
      ...
      {mode === 'daily' && dailyDone && (!current || current.id === dailyQuestion?.id) && selected === null ? (
        ...
        <div className="rounded-xl bg-card2 border border-line p-3 flex flex-col gap-2.5">
          <div className="num flex items-center gap-2 flex-wrap text-xs">
            <span className={`px-2 py-0.5 rounded-full text-white ${MODULE_COLOR[module_]}`}>{module_}</span>
            <span className="text-mut">{current.topic}</span>
            ...
            {['A', 'B', 'C', 'D'].map((L) => {
              ...
              return (
                <button key={L} onClick={() => answer(L)} disabled={!!selected}
                  className={`text-left rounded-lg border px-3 py-2 min-h-[44px] text-sm transition-colors ${press} ${focusRing} ${
                    isRight ? 'border-emerald-500 bg-emerald-400/15'
                    : isWrong ? 'border-rose-500 bg-rose-400/15'
                    : selected ? 'border-line opacity-60'
                    : 'border-line2 hover:bg-veil'
                  }`}>
                  <span className="num font-bold mr-2">{L}</span>{current.options[L]}
                  {isRight && <span className="ml-1 text-emerald-400">✓</span>}
                  {isWrong && <span className="ml-1 text-rose-400">✗</span>}
                </button>
              )
            })}
          </div>
          {selected && (
            <>
              <p className={`text-sm font-medium ${selected === current.correct ? 'text-emerald-400' : 'text-rose-400'}`} role="status">
                {selected === current.correct ? '✓ Correct' : '✗ Incorrect — saved for review'}
              </p>
              <p className="text-sm text-mut">{current.explanation}</p>
        ...
        <p className="text-sm text-center py-4 text-mut">No questions available for this mode</p>
      )}
      ...
      <div className="border-t border-line">
        <Accordion label={`Study history (${history.length})`}>
          {history.length === 0 ? (
            <p className="text-sm text-mut pb-2">No attempts yet — answer a question to start your history</p>
          ) : (
              ...
              {history.slice(0, 10).map((h, i) => (
                <li key={i}>
                  <button onClick={() => { setModule(h.module); setMode('practice'); setSelected(null); setCurrent(byId[h.questionId]) }}
                    ...
                    <span className={h.correct ? 'text-emerald-400' : 'text-rose-400'}>{h.correct ? '✓' : '✗'}</span>
                    <span className={`num font-bold ${MODULE_TEXT[h.module]}`}>{h.module}</span>
                    <span className="text-mut truncate">{h.topic}</span>
                    <span className="num ml-auto text-mut">{h.date.slice(5)}</span>
                  </button>
                </li>
              ))}
            </ul>
        ...
        {!confirmReset ? (
          <GhostBtn danger onClick={() => { setConfirmReset(true); setTimeout(() => setConfirmReset(false), 5000) }}>
            Reset <span className="num">{module_}</span> progress
          </GhostBtn>
        ) : (
          <div className="flex items-center gap-2 text-sm">
            <span className="text-rose-400">Really reset <span className="num">{module_}</span>? This clears <span className="num">{s.done}</span> attempts.</span>
            <button onClick={resetModule} className={`text-rose-400 font-semibold px-2 py-2 min-h-[44px] ${press} ${focusRing}`}>Confirm</button>
